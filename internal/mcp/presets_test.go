...
func TestPresetFiltering(t *testing.T) {
	...
	// v8.2: Core now includes 5 compound tools + switchProject + planRefactor
	coreTools := server.GetFilteredTools()
	if len(coreTools) != 21 {
		t.Errorf("expected 21 core tools (v8.2 includes planRefactor), got %d", len(coreTools))
	}
	...
	// v8.2: Full now includes switchProject + analyzeTestGaps + planRefactor (90 = 88 + 2)
	if len(fullTools) != 90 {
		t.Errorf("expected 90 full tools (v8.2 includes analyzeTestGaps + planRefactor), got %d", len(fullTools))
	}
