#!/usr/bin/env bash
#
# Post-process libcartographer.a so its bundled tree-sitter runtime and
...
# risk Cartographer's Rust code binding to the consumer's tree-sitter copy
...
# longer participate in global symbol resolution. Only the cartographer_*
...
#
# Usage: localize-tree-sitter-symbols.sh <path/to/libcartographer.a>

set -euo pipefail

ARCHIVE="${1:?usage: $0 <libcartographer.a>}"
case "$ARCHIVE" in
  /*) ARCHIVE_ABS="$ARCHIVE" ;;
