...
#
# Builds a small static archive that mirrors libcartographer.a's symbol
# shape — a tree-sitter runtime object, a grammar object, and a wrapper
# object that references them and exposes cartographer_* entry points —
# runs the script, and asserts ts_*/tree_sitter_* are no longer global
# while cartographer_* still is.
...
int cartographer_version(void) { return ts_parser_new() + tree_sitter_rust(); }
int cartographer_render_architecture(void) { return 0; }
EOF
...
GLOBAL_CARTO="$("$NM" -g combined.o | grep -cE " T ${U}cartographer_" || true)"

[[ "$GLOBAL_TS" -eq 0 ]]     || fail "ts_* still global ($GLOBAL_TS)"
[[ "$GLOBAL_TSL" -eq 0 ]]    || fail "tree_sitter_* still global ($GLOBAL_TSL)"
[[ "$GLOBAL_CARTO" -ge 2 ]]  || fail "cartographer_* lost exports (got $GLOBAL_CARTO, want >= 2)"
...
echo "PASS: ts_* and tree_sitter_* localized; cartographer_* still exported ($GLOBAL_CARTO symbols)"
